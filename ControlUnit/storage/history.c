...
void init_history(void)
{
	...
	if (size == 0 || size > EEPROM_END_ADDRESS - HISTORY_ADDRESS)
	{
		clear_history();
	}
}
...
History load_history(void)
{
	...
	history.data = (uint8_t*)malloc(history.size);
	
	// Check if there is a "History overflow".
	if ((((uint16_t) HISTORY_ADDRESS) + start_index + history.size) > (uint16_t) EEPROM_END_ADDRESS)
	{
		// Check how much to read until the end of EEPROM.
		uint16_t read_to_end_size = ((uint16_t) EEPROM_END_ADDRESS) - ((uint16_t) HISTORY_ADDRESS) - start_index;
		
		// Read until end of EEPROM.
		eeprom_read_block(history.data, (const void*)(((uint16_t) HISTORY_ADDRESS) + start_index), read_to_end_size);
		
		// Continue reading the remaining data from the start of EEPROM.
		eeprom_read_block(history.data + read_to_end_size, (const void*)(((uint16_t) HISTORY_ADDRESS)), history.size - read_to_end_size);
	}
	else
	{
		// No overflow, we can just read the history starting at the first history_address.
		eeprom_read_block(history.data, (const void*)((uint16_t) HISTORY_ADDRESS), history.size);
	}
...
void write_measurement(uint8_t value)
{
	...
	// Check if there is a "History overflow".
	if (HISTORY_ADDRESS + start_index + size < EEPROM_END_ADDRESS)
	{
		eeprom_write_byte((uint8_t*) (HISTORY_ADDRESS + start_index + size), value);
		eeprom_write_word((uint16_t*) HISTORY_SIZE_ADDRESS, size + 1);
	}
	else
	{
		eeprom_write_byte((uint8_t*) (HISTORY_ADDRESS + start_index), value);
